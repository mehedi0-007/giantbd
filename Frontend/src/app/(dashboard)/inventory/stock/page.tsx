...
export default function CurrentStockPage() {
  ...
  const { data: batchesData, isLoading: loadingBatches, isFetching: fetchingBatches } = useQuery({
    queryKey: ['inventory-batches', page, pageSize, search],
    queryFn: async () => {
  ...
  const { data: stockData, isLoading: loadingStock, isFetching: fetchingStock } = useQuery({
    queryKey: ['live-stock', page, pageSize, search, warehouseFilter, lowStockOnly],
    queryFn: async () => {
        ...
        <div className="overflow-hidden rounded-2xl border border-slate-200/80 bg-white shadow-xs">
          {loadingBatches ? (
            ...
            <>
              ...
              <DataPagination
                currentPage={page}
                totalPages={batchesData?.total_page || 1}
                totalCount={batchesData?.total || rawBatches.length}
                pageSize={pageSize}
        ...
        <div className="overflow-hidden rounded-2xl border border-slate-200/80 bg-white shadow-xs">
          {loadingStock ? (
            ...
            <>
              ...
              <DataPagination
                currentPage={page}
                totalPages={stockData?.total_page || 1}
                totalCount={stockData?.total || rawStockItems.length}
                pageSize={pageSize}
                onPageChange={(p) => setPage(p)}
                onPageSizeChange={(s) => setPageSize(s)}
                pageSizeOptions={[10, 20, 50, 100]}
